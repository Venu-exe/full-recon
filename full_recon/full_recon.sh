...
# ============================================================
# AUTO-TRIAGE COMPREHENSIVE RESULTS
# ============================================================
section "Running Comprehensive Triage"
...
if [ -n "$TRIAGE_SCRIPT" ]; then
    ...
    log "Generating triage reports in all formats..."
    cd "$OUTDIR"
    
    # Generate all formats
    "$TRIAGE_SCRIPT" "$DOMAIN" --format text 2>/dev/null && success "✓ Text format complete" || warn "Text format failed"
    "$TRIAGE_SCRIPT" "$DOMAIN" --format json 2>/dev/null && success "✓ JSON format complete" || warn "JSON format failed"
    "$TRIAGE_SCRIPT" "$DOMAIN" --format csv 2>/dev/null && success "✓ CSV format complete" || warn "CSV format failed"
    "$TRIAGE_SCRIPT" "$DOMAIN" --format markdown 2>/dev/null && success "✓ Markdown format complete" || warn "Markdown format failed"
    
    cd - > /dev/null
    
    log ""
    log "✓ All triage formats complete!"
    log ""
    log "Available outputs:"
    [ -f "$OUTDIR/triage/priority_list.txt" ] && log "  ✓ Text:     cat recon_${DOMAIN}/triage/priority_list.txt"
    [ -f "$OUTDIR/triage/priority_list.json" ] && log "  ✓ JSON:     cat recon_${DOMAIN}/triage/priority_list.json | jq ."
    [ -f "$OUTDIR/triage/priority_list.csv" ] && log "  ✓ CSV:      cat recon_${DOMAIN}/triage/priority_list.csv"
    [ -f "$OUTDIR/triage/priority_list.md" ] && log "  ✓ Markdown: cat recon_${DOMAIN}/triage/priority_list.md"
    log ""
else
    warn "triage_enhanced.sh not found in ./ or ../"
    log "To enable auto-triage, place triage_enhanced.sh in:"
    log "  - Same directory as full_recon.sh, OR"
    log "  - Parent directory"
fi

log "Full recon complete for $DOMAIN"
